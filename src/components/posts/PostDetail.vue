  ...
  <div class="py-12 bg-gray-100 mt-32">
    <div class="max-w-3xl mx-auto bg-white rounded-lg shadow-lg p-6">
      ...
      <div class="mb-8" v-if="post">
        ...
        <p class="text-gray-500 text-sm">Published on {{ new Date(post.date).toLocaleDateString() }} by {{ getAuthorName(post.user) }}</p>
      </div>
      ...
      if (this.newCommentText.trim()) {
        try {
          const newComment = {
            ...
            author: 'Anonymous', // Replace with actual user name if available
            postId: this.post.id
          };
